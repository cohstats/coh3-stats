...
// prettier-ignore
export const isMobileDeviceCheck = () => {
  let check = false;
  (function (a) {
    if (
...
/**
 * If the position is under 100, don't change, if the position is over 100, round it to the nearest 100.
 * This is because we don't want to create infinite amount of pages which always start on some different position.
 * @param position
 */
export const getCorrectLeaderStartPositions = (position: number) => {
  if (position <= 100) {
    return 1;
  } else {

...
export interface PSNProps {
  label?: React.ReactNode;
  size?: number;
  mode?: "icon-button" | "svg";
}

export const PSNIcon: React.FC<PSNProps> = ({ label, size = 22, mode = "icon-button" }) => {
  const PSNSvg = (
    <svg
      width={size}
      height={size}
  ...
  return <IconButton label={label || "PlayStation Network"}>{PSNSvg}</IconButton>;
};
